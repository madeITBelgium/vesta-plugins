...
add_top_menu_item('Node.JS', '/list?plugin=nodejs', true, "Plugin", 1, 'loadNodejsIndex');

function loadNodejsIndex() {
    $version = null;
    exec('node -v', $version);
        ...
        <div class="l-sort-toolbar clearfix">
          <table>
            <tr>
              ...
              <td>
                <form action="/list?plugin=nodejs" method="post" id="objects">
                <input type="hidden" name="token" value="<?=$_SESSION['token']?>" />
                <div class="l-select">
          ...
          <div class="l-unit-toolbar__col l-unit-toolbar__col--right noselect">
            <div class="actions-panel clearfix">
              <? if ($data[$key]['UPDATED'] == 'no') {
                   echo '<div class="actions-panel__col actions-panel__update shortcut-u" key-action="href"><a href="/list?plugin=nodejs&update='.$key.'&token='.$_SESSION['token'].'">'.__('update').'<i></i></a><span class="shortcut">&nbsp;U</span></div>';
                 }
              ?>
            </div>
            <!-- /.actiona-panel -->
